...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiBody>) {
  ...
  try {
    ...
    if (!rows.length) {
      return res.status(400).json({ code: "INVALID_TOKEN", message: "Geçersiz veya kullanılmış bağlantı." });
    }
    ...
    if (!u.verify_expires_at || new Date(u.verify_expires_at).getTime() < Date.now()) {
      return res.status(400).json({ code: "EXPIRED", message: "Doğrulama bağlantısının süresi dolmuş." });
    }
